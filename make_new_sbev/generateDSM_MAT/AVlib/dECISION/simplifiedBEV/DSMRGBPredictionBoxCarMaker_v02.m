function [DSM_out, LANE_MARK_FLAG_out, EGO_SHAPE_FLAG_out] = DSMRGBPredictionBoxCarMaker_v02(DSM_in, State_trajectory, laneInfoL, laneInfoR, Target_X_pred, LANE_MARK_FLAG_in, EGO_SHAPE_FLAG_in, DSM_PARAM, TRAJ_PARAM, EGO_VEHICLE, TRACKING, SAMPLE_TIME)
% DSMRGBPredictionBoxCarMaker function generates DSM with predicted state as bounding box in RGB image for CarMaker data
%
% [DSM_out, LANE_MARK_FLAG_out, EGO_SHAPE_FLAG_out] = DSMRGBPredictionBoxCarMaker(DSM_in, State_trajectory, laneInfoL, laneInfoR, Target_X_pred, LANE_MARK_FLAG_in, EGO_SHAPE_FLAG_in, DSM_PARAM, TRAJ_PARAM, EGO_VEHICLE, TRACKING, SAMPLE_TIME)
%
...
Target_Exist_in_Input_DSM = 0;

DSM_out = DSM_in;
ch_field = fieldnames(DSM_PARAM.CHANNEL_INFO);
CH_LENGTH = length(DSM_PARAM.CHANNEL_INFO);

if ~isequal(empty_DSM, DSM_out)
    Target_Exist_in_Input_DSM = 1;
end
...
if DSM_PARAM.LANE_MARK.ON
    if LANE_MARK_FLAG_in == 0 && Target_Shape_Exist_Flag == 1
        ...
        x_lane_left = DSM_PARAM.RANGE.X_RANGE;
            ...
            for i_line=1:length(tmp_lane_y)
                if x_lane_left(i_line) >= DSM_PARAM.RANGE.X_MIN && x_lane_left(i_line) <= DSM_PARAM.RANGE.X_MAX...
                    ...
                    if DSM_PARAM.COLLISION_PROBABILITY.ON
                        ...
                        if DSM_PARAM.BACKGROUND_COLOR_WHITE == 1
                            DSM_out(X_LINE_uint8, Y_LINE_uint8, 1) = DSM_PARAM.RGB_MIN;

                            if rem(1,3) == 1 % if remainder = 1, R ch -> current channel number +1, +2 = 0
                                DSM_out(X_LINE_uint8,Y_LINE_uint8,1 + 1) = DSM_PARAM.RGB_MIN;
                                DSM_out(X_LINE_uint8,Y_LINE_uint8,1 + 2) = DSM_PARAM.RGB_MIN;
                            elseif rem(1,3) == 2 % if remainder = 2, G ch -> current channel number -1, +1 = 0
                                DSM_out(X_LINE_uint8,Y_LINE_uint8,1 - 1) = DSM_PARAM.RGB_MIN;
                                DSM_out(X_LINE_uint8,Y_LINE_uint8,1 + 1) = DSM_PARAM.RGB_MIN;
                            elseif rem(1,3) == 3 % if remainder = 3, B ch -> current channel number -2, -1 = 0
                                DSM_out(X_LINE_uint8,Y_LINE_uint8,1 - 2) = DSM_PARAM.RGB_MIN;
                                DSM_out(X_LINE_uint8,Y_LINE_uint8,1 - 1) = DSM_PARAM.RGB_MIN;
                            end
                        end
        ...
        x_lane_right = DSM_PARAM.RANGE.X_RANGE;
...
if DSM_PARAM.PREDICTION.TARGET == 1 && Target_Shape_Exist_Flag == 1
    ...
    if ~isequal(empty_DSM, DSM_out)
        ...
        if sum(Target_X_pred) ~= 0
            ...
            if DSM_PARAM.PREDICTION.OVERLAP_FLAG == 1 % overlap 허용
                ...
                for index_pred = 1:DSM_PARAM.PREDICTION.TARGET_PRED_WINDOW/DSM_PARAM.PREDICTION.TARGET_PRED_SAMPLE_RATE
                    ...
                    if DSM_PARAM.PREDICTION.ALL_SHAPE_FLAG % hollow shape for all prediction time
                        tmp_target_y_vertex = [-State_trajectory(TRAJ_PARAM.WIDTH, end)/2, -State_trajectory(TRAJ_PARAM.WIDTH, end)/2,...
                            State_trajectory(TRAJ_PARAM.WIDTH, end)/2, State_trajectory(TRAJ_PARAM.WIDTH, end)/2, -State_trajectory(TRAJ_PARAM.WIDTH, end)/2];
                        tmp_target_x_vertex = [0, State_trajectory(TRAJ_PARAM.LENGTH, end), State_trajectory(TRAJ_PARAM.LENGTH, end), 0, 0];
                                ...
                                if DSM_PARAM.BACKGROUND_COLOR_WHITE == 1
                                    for i_ch = 1:CH_LENGTH
                                        ...
                                        elseif DSM_PARAM.CHANNEL_INFO(i_ch).TRAJ_STATE == TRAJ_PARAM.COLLISION_PROBABILITY
                                            if DSM_PARAM.PREDICTION.FADING.ON
                                                tmp_white_vector = DSM_PARAM.RGB_MAX - ( Collision_Probability_uint8 - 1 );
                                                fading_factor_step = DSM_PARAM.PREDICTION.FADING.FADING_FACTOR.FADING_VALUE + ( index_pred - 1 ) * DSM_PARAM.PREDICTION.FADING.FADING_FACTOR.FADING_FACTOR_STEP_VALUE;

                                                for tmp_j = 1:length(x_contour_total)
                                                    DSM_out(x_contour_total(tmp_j), y_contour_total(tmp_j), DSM_PARAM.CHANNEL_INFO(i_ch).CHANNEL_NUMBER) = ( Collision_Probability_uint8 - 1 ) + tmp_white_vector * fading_factor_step;
                                                end
                                            else
                                                for tmp_j = 1:length(x_contour_total)
                                        ...
                                        elseif State_trajectory(TRAJ_PARAM.RGB_CYCLIST,end) ~= 0 && DSM_PARAM.CHANNEL_INFO(i_ch).TRAJ_STATE == TRAJ_PARAM.RGB_CYCLIST
                                            for tmp_j = 1:length(x_contour_total)
                                                DSM_out(x_contour_total(tmp_j), y_contour_total(tmp_j), DSM_PARAM.CHANNEL_INFO(i_ch).CHANNEL_NUMBER) = DSM_PARAM.RGB_MAX;
                                            end

                                        elseif State_trajectory(TRAJ_PARAM.RGB_CAR,end) ~= 0 && DSM_PARAM.CHANNEL_INFO(i_ch).TRAJ_STATE == TRAJ_PARAM.RGB_CAR
                                            for tmp_j = 1:length(x_contour_total)
                                                DSM_out(x_contour_total(tmp_j), y_contour_total(tmp_j), DSM_PARAM.CHANNEL_INFO(i_ch).CHANNEL_NUMBER) = DSM_PARAM.RGB_MAX;
                                            end

                                        elseif State_trajectory(TRAJ_PARAM.RGB_PEDESTRIAN,end) ~= 0 && DSM_PARAM.CHANNEL_INFO(i_ch).TRAJ_STATE == TRAJ_PARAM.RGB_PEDESTRIAN
                                            for tmp_j = 1:length(x_contour_total)
                                                DSM_out(x_contour_total(tmp_j), y_contour_total(tmp_j), DSM_PARAM.CHANNEL_INFO(i_ch).CHANNEL_NUMBER) = DSM_PARAM.RGB_MAX;
                                            end
                                        ...
                                        elseif DSM_PARAM.CHANNEL_INFO(i_ch).TRAJ_STATE == TRAJ_PARAM.NA
                                            if DSM_PARAM.PREDICTION.FADING.ON
                                                tmp_white_vector = DSM_PARAM.RGB_MAX - DSM_PARAM.RGB_MIN;
                                                fading_factor_step = DSM_PARAM.PREDICTION.FADING.FADING_FACTOR.FADING_VALUE + ( index_pred - 1 ) * DSM_PARAM.PREDICTION.FADING.FADING_FACTOR.FADING_FACTOR_STEP_VALUE;

                                                for tmp_j = 1:length(x_contour_total)
                                                    DSM_out(x_contour_total(tmp_j), y_contour_total(tmp_j), DSM_PARAM.CHANNEL_INFO(i_ch).CHANNEL_NUMBER) = DSM_PARAM.RGB_MIN + tmp_white_vector * fading_factor_step;
                                                end
                                            else
                                                for tmp_j = 1:length(x_contour_total)
                ...
                if overlap_btw_ego_pred_flag == 1 && prediction_time_step_at_overlap ~= 0 % 자차와 겹치는 예측 시점 존재
                    ...
                    last_nonOverlap_prediction_time_step = DSM_PARAM.PREDICTION.TARGET_PRED_WINDOW / DSM_PARAM.PREDICTION.TARGET_PRED_SAMPLE_RATE;
                ...
                if last_nonOverlap_prediction_time_step ~= 0
                    ...
                    for index_pred_detail = round(last_nonOverlap_prediction_time_step*DSM_PARAM.PREDICTION.TARGET_PRED_SAMPLE_RATE/SAMPLE_TIME) : -1 : round((last_nonOverlap_prediction_time_step-1)*DSM_PARAM.PREDICTION.TARGET_PRED_SAMPLE_RATE/SAMPLE_TIME)+1
                ...
                for index_pred = last_nonOverlap_prediction_time_step : -1 : 1
                    ...
                    if DSM_PARAM.PREDICTION.ALL_SHAPE_FLAG % hollow shape for 자차와 겹치지 않는 prediction time까지 그림
...
if Target_Exist_in_Input_DSM == 0 && Target_Shape_Exist_Flag == 0
