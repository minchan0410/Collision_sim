function [DSM_out] = DSMGrayscale(DSM_in, State_trajectory, DSM_PARAM, TRAJ_PARAM)
% DSMGrayscale function generates DSM with occupancy channel and threat metrics
%
% [DSM_out] = DSMGrayscale(DSM_in, State_trajectory, DSM_PARAM, TRAJ_PARAM)
%
        ...
        if norm([State_trajectory(TRAJ_PARAM.REL_POS_X, index_traj), State_trajectory(TRAJ_PARAM.REL_POS_Y, index_traj)],2) ~= 0 ...
            ...
            elseif DSM_PARAM.BACKGROUND_COLOR_WHITE == 1
                for i_ch = 1:CH_LENGTH
                    ...
                    elseif DSM_PARAM.CHANNEL_INFO(i_ch).TRAJ_STATE == TRAJ_PARAM.I_LAT % I_LAT
                        [~,I_LAT_uint8] = min(abs(State_trajectory(TRAJ_PARAM.I_LAT, index_traj) - DSM_PARAM.RANGE.I_LAT_RANGE));
                        DSM_out(Image_Position_X,Image_Position_Y,DSM_PARAM.CHANNEL_INFO(i_ch).CHANNEL_NUMBER) = DSM_PARAM.RGB_MAX - (I_LAT_uint8-1);
                    end
                end
            end
        end
