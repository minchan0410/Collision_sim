function [SBEV_out, LANE_MARK_FLAG_out, EGO_SHAPE_FLAG_out] = DSMMaxCollisionProbabilityCN7_v1(SBEV_in, State_trajectory, laneInfoL, laneInfoR,...
...
if SBEV_PARAM.LANE_MARK.ON
    if LANE_MARK_FLAG_in == 0 && Target_Shape_Exist_Flag == 1
        ...
        if laneInfoL(FRONT_VISION_LANE.MEASURE.VIEW_END, 1) > SBEV_PARAM.RANGE.X_MAX
            ...
            x_lane_left = [laneInfoL(FRONT_VISION_LANE.MEASURE.VIEWRANGE_START, 1):(SBEV_PARAM.RANGE.X_MAX - SBEV_PARAM.RANGE.X_MIN)/(SBEV_PARAM.IMAGE_HEIGHT - 1):laneInfoL(FRONT_VISION_LANE.MEASURE.VIEW_END, 1)];
        end
        
        if laneInfoR(FRONT_VISION_LANE.MEASURE.VIEW_END, 1) > SBEV_PARAM.RANGE.X_MAX
            ...
            x_lane_right = [laneInfoR(FRONT_VISION_LANE.MEASURE.VIEWRANGE_START, 1):(SBEV_PARAM.RANGE.X_MAX - SBEV_PARAM.RANGE.X_MIN)/(SBEV_PARAM.IMAGE_HEIGHT - 1):laneInfoR(FRONT_VISION_LANE.MEASURE.VIEW_END, 1)];
        end
