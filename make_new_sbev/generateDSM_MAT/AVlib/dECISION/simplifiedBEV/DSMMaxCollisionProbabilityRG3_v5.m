function [SBEV_out, LANE_MARK_FLAG_out, EGO_SHAPE_FLAG_out] = DSMMaxCollisionProbabilityRG3_v5(SBEV_in, State_trajectory, laneInfoL, laneInfoR,...
    Target_X_pred, LANE_MARK_FLAG_in, EGO_SHAPE_FLAG_in, SBEV_PARAM, TRAJ_PARAM, FRONT_VISION_LANE, EGO_VEHICLE, TRACKING, SAMPLE_TIME)
...
% simplifiedBEV3PredictionExpG80_test9 에서 마지막 predicted state가 ROI 경계에 걸칠 때 box가 찌그러지는 문제 수정
% DSMMaxCollisionProbabilityRG3_v1 : simplifiedBEV3PredictionExpG80_test10 에서 max collision probability를 B ch에 입력
% DSMMaxCollisionProbabilityRG3_v2 : DSMMaxCollisionProbabilityRG3_v1 에서 prediction option을 반영하도록 수정
% DSMMaxCollisionProbabilityRG3_v3 : DSMMaxCollisionProbabilityRG3_v2 에서 trajectory에 fading 적용 가능하도록 수정
% DSMMaxCollisionProbabilityRG3_v4 :  DSMMaxCollisionProbabilityRG3_v3 에서 예측 위치를 궤적으로 나타낼 때 과거 trajectory option 적용되던 부분 안되게 수정
% DSMMaxCollisionProbabilityRG3_v5 : DSMMaxCollisionProbabilityRG3_v4 에서 궤적에 위험도와 fading 동시 적용 가능하도록 수정
...
if SBEV_PARAM.LANE_MARK.ON
    if LANE_MARK_FLAG_in == 0 && Target_Shape_Exist_Flag == 1
        ...
        if laneInfoL(FRONT_VISION_LANE.MEASURE.VIEWRANGE, 1) > SBEV_PARAM.RANGE.X_MAX
            x_lane_left = [laneInfoL(FRONT_VISION_LANE.MEASURE.VIEWRANGE_START, 1):(SBEV_PARAM.RANGE.X_MAX - SBEV_PARAM.RANGE.X_MIN)/(SBEV_PARAM.IMAGE_HEIGHT - 1):SBEV_PARAM.RANGE.X_MAX];
        else
            x_lane_left = [laneInfoL(FRONT_VISION_LANE.MEASURE.VIEWRANGE_START, 1):(SBEV_PARAM.RANGE.X_MAX - SBEV_PARAM.RANGE.X_MIN)/(SBEV_PARAM.IMAGE_HEIGHT - 1):laneInfoL(FRONT_VISION_LANE.MEASURE.VIEWRANGE, 1)];
        end
        
        if laneInfoR(FRONT_VISION_LANE.MEASURE.VIEWRANGE, 1) > SBEV_PARAM.RANGE.X_MAX
            x_lane_right = [laneInfoR(FRONT_VISION_LANE.MEASURE.VIEWRANGE_START, 1):(SBEV_PARAM.RANGE.X_MAX - SBEV_PARAM.RANGE.X_MIN)/(SBEV_PARAM.IMAGE_HEIGHT - 1):SBEV_PARAM.RANGE.X_MAX];
        else
            x_lane_right = [laneInfoR(FRONT_VISION_LANE.MEASURE.VIEWRANGE_START, 1):(SBEV_PARAM.RANGE.X_MAX - SBEV_PARAM.RANGE.X_MIN)/(SBEV_PARAM.IMAGE_HEIGHT - 1):laneInfoR(FRONT_VISION_LANE.MEASURE.VIEWRANGE, 1)];
        end
...
%                                                         end
                ...
                ego_vertex = [-EGO_VEHICLE.WIDTH/2 - ROI_margin2ego, -EGO_VEHICLE.WIDTH/2 - ROI_margin2ego, EGO_VEHICLE.WIDTH/2 + ROI_margin2ego, EGO_VEHICLE.WIDTH/2 + ROI_margin2ego, -EGO_VEHICLE.WIDTH/2 - ROI_margin2ego;
                    -EGO_VEHICLE.LENGTH, 0, 0, -EGO_VEHICLE.LENGTH, -EGO_VEHICLE.LENGTH];
