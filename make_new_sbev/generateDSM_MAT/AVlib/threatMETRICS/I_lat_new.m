function [I_lat_out, I_long, TTC_out, x_p, d_br, d_w,  DLC, TLC]...
    = I_lat_new(rel_pos_x, rel_pos_y, rel_vel_x, rel_vel_y, Va,Vb,heading_angle, target_width, target_length, ego_length, distance_to_leftlane, distance_to_rightlane, I_LAT_PARAM)
...
if p_long ~= P_LONG_DEFAULT_VALUE
    if rel_vel_x < 0
        if rel_pos_x > 0 && p_long > 0 || rel_pos_x < 0 && p_long < 0 % TTC for front vehicle || TTC for rear vehicle
            TTC_out = p_long/(-rel_vel_x);
...
I_long = max((X_MAX - x_p)/(X_MAX - X_THRESHOLD), abs(1/TTC_out)/abs(TTC_INVERSE_THRESHOLD));
% if I_long>1
%     I_long=1;
% end
...
if TLC < 0 % safe
    ...
    I_lat_out = min(I_long,1) * min(TLC_THRESHOLD/TLC,1);
end
