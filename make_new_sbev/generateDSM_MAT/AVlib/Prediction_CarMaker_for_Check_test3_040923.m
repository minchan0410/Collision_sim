...
if TARGET_PRED_EKF_CTRV
    ...
    X_est = zeros(STATE_NUMBER, Traffic_Number, length(sim_time)); % [x, y, vx, vy, heading angular rate]' + [heading angle, width, length, classification, motion]'
    P_est = zeros(TRACKING_STATE_NUMBER, TRACKING_STATE_NUMBER, Traffic_Number, length(sim_time));
    
    X_pred = zeros(STATE_NUMBER, Traffic_Number, length(sim_time)); % [x, y, vx, vy, heading angular rate]' + [heading angle, width, length, classification, motion]'
    P_pred = zeros(TRACKING_STATE_NUMBER, TRACKING_STATE_NUMBER, Traffic_Number, length(sim_time));
    
    z_CTRV = zeros(TRACKING_STATE_NUMBER - 1, Traffic_Number, length(sim_time)); % [x, y, vx, vy]'
    X_updated = zeros(STATE_NUMBER, Traffic_Number, length(sim_time)); % [x, y, vx, vy, heading angular rate]' + [heading angle, width, length, classification, motion]'
    P_updated = zeros(TRACKING_STATE_NUMBER, TRACKING_STATE_NUMBER, Traffic_Number, length(sim_time));
    
    X_pred_window = zeros(STATE_NUMBER, length(sim_time), TARGET_PRED_WINDOW/SAMPLE_TIME, Traffic_Number); % [x, y, vx, vy, heading angular rate]' + [heading angle, width, length, classification, motion]'
    P_pred_window = zeros(TRACKING_STATE_NUMBER, TRACKING_STATE_NUMBER, length(sim_time), TARGET_PRED_WINDOW/SAMPLE_TIME, Traffic_Number);

    Association_Map_Total = zeros(Traffic_Number, length(sim_time));

    X_pred_window_SBEV = zeros(STATE_NUMBER, length(sim_time), TARGET_PRED_WINDOW/TARGET_PRED_SAMPLE_RATE, Traffic_Number);
    ...
    for index_time = 1:Test_end_index
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            ...
            for track_number_k_1 = 1:Traffic_Number
                if sum(Association_Map_k_1(track_number_k_1, 1)) ~= 0
                    ...
                    if sorted_track_number_k_1(1) == track_number_k_1 && ...
                        ...
                        z_CTRV(:,track_number_k_1,index_time) = [Fusion_Track([FUSION_TRACK.MEASURE.REL_POS_X, FUSION_TRACK.MEASURE.REL_POS_Y, FUSION_TRACK.MEASURE.REL_VEL_X, FUSION_TRACK.MEASURE.REL_VEL_Y], sorted_track_number(1), index_time)]...
            ...
            for track_number = 1:Traffic_Number
                if Fusion_Track(FUSION_TRACK.MEASURE.REL_POS_X, track_number, index_time)^2 + Fusion_Track(FUSION_TRACK.MEASURE.REL_POS_Y, track_number, index_time)^2 ~= 0
                    % SBEV ROI
                    if Fusion_Track(FUSION_TRACK.MEASURE.REL_POS_X, track_number, index_time) >= X_MIN && Fusion_Track(FUSION_TRACK.MEASURE.REL_POS_X, track_number, index_time) <= X_MAX ...
                            && Fusion_Track(FUSION_TRACK.MEASURE.REL_POS_Y, track_number, index_time) >= Y_MIN && Fusion_Track(FUSION_TRACK.MEASURE.REL_POS_Y, track_number, index_time) <= Y_MAX
                ...
                if Fusion_Track(FUSION_TRACK.MEASURE.REL_POS_X, track_number, index_time) >= X_MIN && Fusion_Track(FUSION_TRACK.MEASURE.REL_POS_X, track_number, index_time) <= X_MAX ...
                    ...
                    for index_pred = 1:TARGET_PRED_WINDOW/SAMPLE_TIME
                        if index_pred == 1
                            ...
                            % [x, y, vx, vy, heading angular rate]'
                            X_pred_window(TRACKING.REL_POS_X, index_time, index_pred, track_number) = Fusion_Track(FUSION_TRACK.MEASURE.REL_POS_X, track_number, index_time) + ...
                                Fusion_Track(FUSION_TRACK.MEASURE.REL_VEL_X, track_number, index_time) / tmp_heading_angle_rate * sin( tmp_heading_angle_rate * SAMPLE_TIME ) - ...
                                Fusion_Track(FUSION_TRACK.MEASURE.REL_VEL_Y, track_number, index_time) / tmp_heading_angle_rate * ( 1 - cos( tmp_heading_angle_rate * SAMPLE_TIME ) ) ;

                            X_pred_window(TRACKING.REL_POS_Y, index_time, index_pred, track_number) = Fusion_Track(FUSION_TRACK.MEASURE.REL_POS_Y, track_number, index_time) + ...
                                Fusion_Track(FUSION_TRACK.MEASURE.REL_VEL_X, track_number, index_time) / tmp_heading_angle_rate * ( 1 - cos( tmp_heading_angle_rate * SAMPLE_TIME ) ) + ...
                                Fusion_Track(FUSION_TRACK.MEASURE.REL_VEL_Y, track_number, index_time) / tmp_heading_angle_rate * sin( tmp_heading_angle_rate * SAMPLE_TIME );

                            X_pred_window(TRACKING.REL_VEL_X, index_time, index_pred, track_number) = Fusion_Track(FUSION_TRACK.MEASURE.REL_VEL_X, track_number, index_time) * cos( tmp_heading_angle_rate * SAMPLE_TIME ) - ...
                                                                                                        Fusion_Track(FUSION_TRACK.MEASURE.REL_VEL_Y, track_number, index_time) * sin( tmp_heading_angle_rate * SAMPLE_TIME );

                            X_pred_window(TRACKING.REL_VEL_Y, index_time, index_pred, track_number) = Fusion_Track(FUSION_TRACK.MEASURE.REL_VEL_X, track_number, index_time) * sin( tmp_heading_angle_rate * SAMPLE_TIME ) + ...
                                                                                                        Fusion_Track(FUSION_TRACK.MEASURE.REL_VEL_Y, track_number, index_time) * cos( tmp_heading_angle_rate * SAMPLE_TIME );
                            ...
                            dx_dw = Fusion_Track(FUSION_TRACK.MEASURE.REL_VEL_X, track_number, index_time) / tmp_heading_angle_rate * cos( tmp_heading_angle_rate * SAMPLE_TIME ) * SAMPLE_TIME - ...
                                Fusion_Track(FUSION_TRACK.MEASURE.REL_VEL_X, track_number, index_time) / tmp_heading_angle_rate^2 * sin( tmp_heading_angle_rate * SAMPLE_TIME ) - ...
                                Fusion_Track(FUSION_TRACK.MEASURE.REL_VEL_Y, track_number, index_time) / tmp_heading_angle_rate * sin( tmp_heading_angle_rate * SAMPLE_TIME ) * SAMPLE_TIME + ...
                                Fusion_Track(FUSION_TRACK.MEASURE.REL_VEL_Y, track_number, index_time) / tmp_heading_angle_rate^2 * ( 1 - cos( tmp_heading_angle_rate * SAMPLE_TIME ) );
                            ...
                            dy_dw = Fusion_Track(FUSION_TRACK.MEASURE.REL_VEL_X, track_number, index_time) / tmp_heading_angle_rate * sin( tmp_heading_angle_rate * SAMPLE_TIME ) * SAMPLE_TIME - ...
                                Fusion_Track(FUSION_TRACK.MEASURE.REL_VEL_X, track_number, index_time) / tmp_heading_angle_rate^2 * ( 1 - cos( tmp_heading_angle_rate * SAMPLE_TIME ) ) + ...
                                Fusion_Track(FUSION_TRACK.MEASURE.REL_VEL_Y, track_number, index_time) / tmp_heading_angle_rate * cos( tmp_heading_angle_rate * SAMPLE_TIME ) * SAMPLE_TIME - ...
                                Fusion_Track(FUSION_TRACK.MEASURE.REL_VEL_Y, track_number, index_time) / tmp_heading_angle_rate^2 * sin( tmp_heading_angle_rate * SAMPLE_TIME );

                            dvx_dvx = cos( tmp_heading_angle_rate * SAMPLE_TIME );
                            dvx_dvy = - sin( tmp_heading_angle_rate * SAMPLE_TIME );
                            dvx_dw = - Fusion_Track(FUSION_TRACK.MEASURE.REL_VEL_X, track_number, index_time) * sin( tmp_heading_angle_rate * SAMPLE_TIME ) * SAMPLE_TIME - ...
                                Fusion_Track(FUSION_TRACK.MEASURE.REL_VEL_Y, track_number, index_time) * cos( tmp_heading_angle_rate * SAMPLE_TIME ) * SAMPLE_TIME;

                            dvy_dvx = sin( tmp_heading_angle_rate * SAMPLE_TIME );
                            dvy_dvy = cos( tmp_heading_angle_rate * SAMPLE_TIME );
                            dvy_dw = Fusion_Track(FUSION_TRACK.MEASURE.REL_VEL_X, track_number, index_time) * cos( tmp_heading_angle_rate * SAMPLE_TIME ) * SAMPLE_TIME - ...
                                Fusion_Track(FUSION_TRACK.MEASURE.REL_VEL_Y, track_number, index_time) * sin( tmp_heading_angle_rate * SAMPLE_TIME ) * SAMPLE_TIME;
...
if TARGET_PRED_UKF_CTRV
    ...
    X_sigma = zeros(TRACKING_STATE_NUMBER, 2*TRACKING_STATE_NUMBER + 1, Traffic_Number, length(sim_time)); % [x, y, vx, vy, heading angular rate]'
    W = zeros(2*TRACKING_STATE_NUMBER + 1, Traffic_Number, length(sim_time));

    X_sigma_pred = zeros(TRACKING_STATE_NUMBER, 2*TRACKING_STATE_NUMBER + 1, Traffic_Number, length(sim_time)); % [x, y, vx, vy, heading angular rate]'
    X_pred = zeros(STATE_NUMBER, Traffic_Number, length(sim_time)); % [x, y, vx, vy, heading angular rate]' + [heading angle, width, length, classification, motion]'
    P_pred = zeros(TRACKING_STATE_NUMBER, TRACKING_STATE_NUMBER, Traffic_Number, length(sim_time));
                ...
                if Fusion_Track(FUSION_TRACK.MEASURE.REL_POS_X, track_number, index_time) >= X_MIN && Fusion_Track(FUSION_TRACK.MEASURE.REL_POS_X, track_number, index_time) <= X_MAX ...
                    ...
                    for index_pred = 1:TARGET_PRED_WINDOW/SAMPLE_TIME
                        if index_pred == 1
                            ...
                            while tmp_i <= 2*TRACKING_STATE_NUMBER
                                if tmp_i == 0
                                    tmp_X_sigma(:, 1) = [Fusion_Track([FUSION_TRACK.MEASURE.REL_POS_X, FUSION_TRACK.MEASURE.REL_POS_Y, FUSION_TRACK.MEASURE.REL_VEL_X, FUSION_TRACK.MEASURE.REL_VEL_Y], track_number, index_time); tmp_heading_angle_rate];
                                    tmp_W(tmp_i + 1, 1) = KAPPA / (KAPPA + TRACKING_STATE_NUMBER);
                                elseif tmp_i <= TRACKING_STATE_NUMBER
                                    tmp_X_sigma(:, tmp_i + 1) = [Fusion_Track([FUSION_TRACK.MEASURE.REL_POS_X, FUSION_TRACK.MEASURE.REL_POS_Y, FUSION_TRACK.MEASURE.REL_VEL_X, FUSION_TRACK.MEASURE.REL_VEL_Y], track_number, index_time); tmp_heading_angle_rate] + L(tmp_i, :)';
                                    tmp_W(tmp_i + 1, 1) = 0.5 / (KAPPA + TRACKING_STATE_NUMBER);
                                else
                                    tmp_X_sigma(:, tmp_i + 1) =  [Fusion_Track([FUSION_TRACK.MEASURE.REL_POS_X, FUSION_TRACK.MEASURE.REL_POS_Y, FUSION_TRACK.MEASURE.REL_VEL_X, FUSION_TRACK.MEASURE.REL_VEL_Y], track_number, index_time); tmp_heading_angle_rate] - L(tmp_i - TRACKING_STATE_NUMBER, :)';
                                    tmp_W(tmp_i + 1, 1) = 0.5 / (KAPPA + TRACKING_STATE_NUMBER);
                                end
                                tmp_i = tmp_i + 1;
                            end
...
if TARGET_PRED_IMM_EKF_CTRV_CV
    ...
    X_CTRV_pred_window = zeros(STATE_NUMBER, length(sim_time), TARGET_PRED_WINDOW/SAMPLE_TIME, Traffic_Number); % [x, y, vx, vy, heading angular rate]' + [heading angle, width, length, classification, motion]'
    P_CTRV_pred_window = zeros(TRACKING_STATE_NUMBER, TRACKING_STATE_NUMBER, length(sim_time), TARGET_PRED_WINDOW/SAMPLE_TIME, Traffic_Number);

    X_CV_pred_window = zeros(STATE_NUMBER, length(sim_time), TARGET_PRED_WINDOW/SAMPLE_TIME, Traffic_Number); % [x, y, vx, vy, heading angular rate]' + [heading angle, width, length, classification, motion]'
    P_CV_pred_window = zeros(CV_TRACKING_STATE_NUMBER, CV_TRACKING_STATE_NUMBER, length(sim_time), TARGET_PRED_WINDOW/SAMPLE_TIME, Traffic_Number);
    ...
    for index_time = 1:Test_end_index
        ...
        % Initialization
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        Y = zeros(STATE_NUMBER, Traffic_Number); % measurement
        ...
        if index_time == 1
            model_CTRV_probability_k_1 = MODEL_CTRV_INITIAL_PROBABILITY * ones(Traffic_Number, 1);
            model_CV_probability_k_1 = MODEL_CV_INITIAL_PROBILITY * ones(Traffic_Number, 1);


            X_CTRV_k_1 = zeros(STATE_NUMBER, Traffic_Number);
            P_CTRV_k_1 = zeros(TRACKING_STATE_NUMBER, TRACKING_STATE_NUMBER, Traffic_Number);

            X_CV_k_1 = zeros(STATE_NUMBER, Traffic_Number);
            P_CV_k_1 = zeros(TRACKING_STATE_NUMBER, TRACKING_STATE_NUMBER, Traffic_Number);
                ...
                for track_number = 1:Traffic_Number
                    if norm([Fusion_Track(FUSION_TRACK.MEASURE.REL_POS_X, track_number, index_time), Fusion_Track(FUSION_TRACK.MEASURE.REL_POS_Y, track_number, index_time)], 2) ~= 0
                        ...
                        if Fusion_Track(FUSION_TRACK.SHAPE_ATTRIBUTE.SHAPE, track_number, index_time) == Fusion_Track_k_1(TRACKING.SHAPE, track_number_k_1)
                            ...
                            tmp_residual(1, track_number, track_number_k_1) = Fusion_Track_k_1(TRACKING.REL_POS_Y, track_number_k_1) - Fusion_Track(FUSION_TRACK.MEASURE.REL_POS_Y, track_number, index_time); % y
                            tmp_residual(2, track_number, track_number_k_1) = Fusion_Track_k_1(TRACKING.REL_POS_X, track_number_k_1) - Fusion_Track(FUSION_TRACK.MEASURE.REL_POS_X, track_number, index_time); % x
                            tmp_residual(3, track_number, track_number_k_1) = Fusion_Track_k_1(TRACKING.REL_VEL_Y, track_number_k_1) - Fusion_Track(FUSION_TRACK.MEASURE.REL_VEL_Y, track_number, index_time); % vy
                            tmp_residual(4, track_number, track_number_k_1) = Fusion_Track_k_1(TRACKING.REL_VEL_X, track_number_k_1) - Fusion_Track(FUSION_TRACK.MEASURE.REL_VEL_X, track_number, index_time); % vx
        ...
        for track_number_k_1 = 1:Traffic_Number
            if sum(Association_Map_k_1(track_number_k_1, 1)) ~= 0
                ...
                if sorted_track_number_k_1(1) == track_number_k_1 && ...
                    ...
                    Y(TRACKING.REL_POS_X, track_number_k_1) = Fusion_Track(FUSION_TRACK.MEASURE.REL_POS_X, sorted_track_number(1), index_time);
                    Y(TRACKING.REL_POS_Y, track_number_k_1) = Fusion_Track(FUSION_TRACK.MEASURE.REL_POS_Y, sorted_track_number(1), index_time);
                    Y(TRACKING.REL_VEL_X, track_number_k_1) = Fusion_Track(FUSION_TRACK.MEASURE.REL_VEL_X, sorted_track_number(1), index_time);
                    Y(TRACKING.REL_VEL_Y, track_number_k_1) = Fusion_Track(FUSION_TRACK.MEASURE.REL_VEL_Y, sorted_track_number(1), index_time);
        ...
        [model_CTRV_probability_pred, X_CTRV_k_1_mixed, P_CTRV_k_1_mixed, model_CV_probability_pred, X_CV_k_1_mixed, P_CV_k_1_mixed] = interaction(model_CTRV_probability_k_1, model_CV_probability_k_1, X_CTRV_k_1, X_CV_k_1,...
                                                                                P_CTRV_k_1, P_CV_k_1, MODEL_TRANSITION_PROBABILITY, Traffic_Number, TRACKING);

        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        % Model Individual Filtering
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        [X_CTRV_k, P_CTRV_k, lambda_CTRV] = EKF_CTRV_model_state5(X_CTRV_k_1_mixed, P_CTRV_k_1_mixed, Y, Association_Map_Total(:, index_time), H_CTRV, Q_CTRV, R_CTRV, Traffic_Number, TRACKING, SAMPLE_TIME);

        [X_CV_k, P_CV_k, lambda_CV] = KF_CV_model_state5(X_CV_k_1_mixed, P_CV_k_1_mixed, Y, Association_Map_Total(:, index_time), A_CV, H_CV, Q_CV, R_CV, Traffic_Number, TRACKING);

        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        % Combination
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        [model_CTRV_probability_k, model_CV_probability_k, X_combined, P_combined] = combination(model_CTRV_probability_pred, X_CTRV_k, P_CTRV_k, lambda_CTRV, model_CV_probability_pred, X_CV_k, P_CV_k, lambda_CV, Traffic_Number, TRACKING);
                ...
                if Fusion_Track(FUSION_TRACK.MEASURE.REL_POS_X, track_number, index_time) >= X_MIN && Fusion_Track(FUSION_TRACK.MEASURE.REL_POS_X, track_number, index_time) <= X_MAX ...
                    ...
                    if Track_Assigned_Flag == 0
                        ...
                        if sum(Association_Map_Total(track_number, index_time)) == 0
                            ...
                            X_CTRV_k(TRACKING.REL_POS_X, track_number) = Fusion_Track(FUSION_TRACK.MEASURE.REL_POS_X, track_number, index_time);
                            X_CTRV_k(TRACKING.REL_POS_Y, track_number) = Fusion_Track(FUSION_TRACK.MEASURE.REL_POS_Y, track_number, index_time);
                            X_CTRV_k(TRACKING.REL_VEL_X, track_number) = Fusion_Track(FUSION_TRACK.MEASURE.REL_VEL_X, track_number, index_time);
                            X_CTRV_k(TRACKING.REL_VEL_Y, track_number) = Fusion_Track(FUSION_TRACK.MEASURE.REL_VEL_Y, track_number, index_time);
                            ...
                            X_CV_k(TRACKING.REL_POS_X, track_number) = Fusion_Track(FUSION_TRACK.MEASURE.REL_POS_X, track_number, index_time);
                            X_CV_k(TRACKING.REL_POS_Y, track_number) = Fusion_Track(FUSION_TRACK.MEASURE.REL_POS_Y, track_number, index_time);
                            X_CV_k(TRACKING.REL_VEL_X, track_number) = Fusion_Track(FUSION_TRACK.MEASURE.REL_VEL_X, track_number, index_time);
                            X_CV_k(TRACKING.REL_VEL_Y, track_number) = Fusion_Track(FUSION_TRACK.MEASURE.REL_VEL_Y, track_number, index_time);
                ...
                if Fusion_Track(FUSION_TRACK.MEASURE.REL_POS_X, track_number, index_time) >= X_MIN && Fusion_Track(FUSION_TRACK.MEASURE.REL_POS_X, track_number, index_time) <= X_MAX ...
                    ...
                    for index_pred = 1:TARGET_PRED_WINDOW/SAMPLE_TIME
                        if index_pred == 1
                            ...
                            X_CTRV_for_prediction(TRACKING.REL_POS_X, 1) = Fusion_Track(FUSION_TRACK.MEASURE.REL_POS_X, track_number, index_time);
                            X_CTRV_for_prediction(TRACKING.REL_POS_Y, 1) = Fusion_Track(FUSION_TRACK.MEASURE.REL_POS_Y, track_number, index_time);
                            X_CTRV_for_prediction(TRACKING.REL_VEL_X, 1) = Fusion_Track(FUSION_TRACK.MEASURE.REL_VEL_X, track_number, index_time);
                            X_CTRV_for_prediction(TRACKING.REL_VEL_Y, 1) = Fusion_Track(FUSION_TRACK.MEASURE.REL_VEL_Y, track_number, index_time);
...
if TARGET_PRED_IMM_EKF_CTRV_CV_CA
    ...
    X_est = zeros(STATE_NUMBER, Traffic_Number, length(sim_time)); % [x, y, vx, vy, ax, ay, heading angular rate]' + [heading angle, width, length, classification, motion]'
    P_est = zeros(TRACKING_STATE_NUMBER, TRACKING_STATE_NUMBER, Traffic_Number, length(sim_time));
    ...
    X_pred_window = zeros(STATE_NUMBER, length(sim_time), TARGET_PRED_WINDOW/SAMPLE_TIME, Traffic_Number); % [x, y, vx, vy, ax, ay, heading angular rate]' + [heading angle, width, length, classification, motion]'
    P_pred_window = zeros(TRACKING_STATE_NUMBER, TRACKING_STATE_NUMBER, length(sim_time), TARGET_PRED_WINDOW/SAMPLE_TIME, Traffic_Number);

    X_CTRV_pred_window = zeros(STATE_NUMBER, length(sim_time), TARGET_PRED_WINDOW/SAMPLE_TIME, Traffic_Number); % [x, y, vx, vy, ax, ay, heading angular rate]' + [heading angle, width, length, classification, motion]'
    P_CTRV_pred_window = zeros(CTRV_TRACKING_STATE_NUMBER, CTRV_TRACKING_STATE_NUMBER, length(sim_time), TARGET_PRED_WINDOW/SAMPLE_TIME, Traffic_Number);

    X_CV_pred_window = zeros(STATE_NUMBER, length(sim_time), TARGET_PRED_WINDOW/SAMPLE_TIME, Traffic_Number); % [x, y, vx, vy, ax, ay, heading angular rate]' + [heading angle, width, length, classification, motion]'
    P_CV_pred_window = zeros(CV_TRACKING_STATE_NUMBER, CV_TRACKING_STATE_NUMBER, length(sim_time), TARGET_PRED_WINDOW/SAMPLE_TIME, Traffic_Number);

    X_CA_pred_window = zeros(STATE_NUMBER, length(sim_time), TARGET_PRED_WINDOW/SAMPLE_TIME, Traffic_Number); % [x, y, vx, vy, ax, ay, heading angular rate]' + [heading angle, width, length, classification, motion]'
    P_CA_pred_window = zeros(CA_TRACKING_STATE_NUMBER, CA_TRACKING_STATE_NUMBER, length(sim_time), TARGET_PRED_WINDOW/SAMPLE_TIME, Traffic_Number);


    P_est_CTRV = zeros(CTRV_TRACKING_STATE_NUMBER, CTRV_TRACKING_STATE_NUMBER, Traffic_Number, length(sim_time));
    P_est_CV = zeros(CV_TRACKING_STATE_NUMBER, CV_TRACKING_STATE_NUMBER, Traffic_Number, length(sim_time));
    P_est_CA = zeros(CA_TRACKING_STATE_NUMBER, CA_TRACKING_STATE_NUMBER, Traffic_Number, length(sim_time));
...
%     R_CA = 0.2*eye(CA_TRACKING_STATE_NUMBER - 1, CA_TRACKING_STATE_NUMBER - 1);
    ...
    model_CTRV_probability_total = zeros(length(sim_time), Traffic_Number);
    model_CV_probability_total = zeros(length(sim_time), Traffic_Number);
    model_CA_probability_total = zeros(length(sim_time), Traffic_Number);
    ...
    for index_time = 1:Test_end_index
        ...
        if index_time == 1
            model_CTRV_probability_k_1 = MODEL_CTRV_INITIAL_PROBABILITY * ones(Traffic_Number, 1);
            model_CV_probability_k_1 = MODEL_CV_INITIAL_PROBILITY * ones(Traffic_Number, 1);
            model_CA_probability_k_1 = MODEL_CA_INITIAL_PROBILITY * ones(Traffic_Number, 1);

            X_CTRV_k_1 = zeros(STATE_NUMBER, Traffic_Number);
            P_CTRV_k_1 = zeros(TRACKING_STATE_NUMBER, TRACKING_STATE_NUMBER, Traffic_Number); % [x, y, vx, vy, ax, ay, heading angular rate]' X [x, y, vx, vy, ax, ay, heading angular rate]'

            X_CV_k_1 = zeros(STATE_NUMBER, Traffic_Number);
            P_CV_k_1 = zeros(TRACKING_STATE_NUMBER, TRACKING_STATE_NUMBER, Traffic_Number);

            X_CA_k_1 = zeros(STATE_NUMBER, Traffic_Number);
            P_CA_k_1 = zeros(TRACKING_STATE_NUMBER, TRACKING_STATE_NUMBER, Traffic_Number);
        ...
        [model_CTRV_probability_pred, X_CTRV_k_1_mixed, P_CTRV_k_1_mixed, model_CV_probability_pred, X_CV_k_1_mixed, P_CV_k_1_mixed, model_CA_probability_pred, X_CA_k_1_mixed, P_CA_k_1_mixed] =...
            interaction3model(model_CTRV_probability_k_1, model_CV_probability_k_1, model_CA_probability_k_1, X_CTRV_k_1, X_CV_k_1, X_CA_k_1, ...
                                                                                P_CTRV_k_1, P_CV_k_1, P_CA_k_1, MODEL_TRANSITION_PROBABILITY, Traffic_Number, TRACKING);
...
%                                                                                 P_CTRV_k_1, P_CV_k_1, P_CA_k_1, MODEL_TRANSITION_PROBABILITY, Traffic_Number, TRACKING);
        ...
        [X_CV_k, P_CV_k, lambda_CV] = KF_CV_model_state7(X_CV_k_1_mixed, P_CV_k_1_mixed, Y, Association_Map_Total(:, index_time), A_CV, H_CV, Q_CV, R_CV, Traffic_Number, TRACKING);

        execution_time_CV_filtering(index_time, 1) = toc;

        tic

        [X_CA_k, P_CA_k, lambda_CA] = KF_CA_model_state7(X_CA_k_1_mixed, P_CA_k_1_mixed, Y, Association_Map_Total(:, index_time), A_CA, H_CA, Q_CA, R_CA, Traffic_Number, TRACKING);

        execution_time_CA_filtering(index_time, 1) = toc;

        tic
        
        [X_CTRV_k, P_CTRV_k, lambda_CTRV] = EKF_CTRV_model_state7(X_CTRV_k_1_mixed, P_CTRV_k_1_mixed, Y, Association_Map_Total(:, index_time), H_CTRV, Q_CTRV, R_CTRV, Traffic_Number, TRACKING, SAMPLE_TIME);
        ...
        [model_CTRV_probability_k, model_CV_probability_k, model_CA_probability_k, X_combined, P_combined] =...
            ...
            model_CA_probability_pred, X_CA_k, P_CA_k, lambda_CA, Traffic_Number, TRACKING);
                ...
                if ( (min(target_y_vertex) >= Y_MIN && min(target_y_vertex) <= Y_MAX) || (max(target_y_vertex) >= Y_MIN && max(target_y_vertex) <= Y_MAX) ) ...
                    ...
                    if Track_Assigned_Flag == 0
                        ...
                        if sum(Association_Map_Total(track_number, index_time)) == 0
                            ...
                            X_CA_k(TRACKING.REL_POS_X, track_number) = Fusion_Track(FUSION_TRACK.MEASURE.REL_POS_X, track_number, index_time);
                            X_CA_k(TRACKING.REL_POS_Y, track_number) = Fusion_Track(FUSION_TRACK.MEASURE.REL_POS_Y, track_number, index_time);
                            X_CA_k(TRACKING.REL_VEL_X, track_number) = Fusion_Track(FUSION_TRACK.MEASURE.REL_VEL_X, track_number, index_time);
                            X_CA_k(TRACKING.REL_VEL_Y, track_number) = Fusion_Track(FUSION_TRACK.MEASURE.REL_VEL_Y, track_number, index_time);
...
%                                 tmp_y_f = (EGO_VEHICLE.EGO_WIDTH + Fusion_Track(FUSION_TRACK.MEASURE.LENGTH, track_number, index_time))/2;
%                                 tmp_y_i = -(EGO_VEHICLE.EGO_WIDTH + Fusion_Track(FUSION_TRACK.MEASURE.LENGTH, track_number, index_time))/2;
...
if TARGET_PRED_CTRV_for_IMM
    ...
    for index_time = 1:Test_end_index
        ...
        % EKF CTRV Filtering
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        [X_CTRV_k, P_CTRV_k, lambda_CTRV] = EKF_CTRV_model_state7(Fusion_Track_k_1, P_Fusion_Track_k_1, Y, Association_Map_Total(:, index_time), H_CTRV, Q_CTRV, R_CTRV, Traffic_Number, TRACKING, SAMPLE_TIME);
        ...
        for track_number = 1:Traffic_Number
            if Fusion_Track(FUSION_TRACK.MEASURE.REL_POS_X, track_number, index_time)^2 + Fusion_Track(FUSION_TRACK.MEASURE.REL_POS_Y, track_number, index_time)^2 ~= 0
                ...
                if Fusion_Track(FUSION_TRACK.MEASURE.REL_POS_X, track_number, index_time) >= X_MIN && Fusion_Track(FUSION_TRACK.MEASURE.REL_POS_X, track_number, index_time) <= X_MAX ...
                    ...
                    if Track_Assigned_Flag == 0
                        for init_track_number = 1:Traffic_Number
                            if sum(Association_Map_Total(init_track_number, index_time)) == 0
                                if Fusion_Track(FUSION_TRACK.SHAPE_ATTRIBUTE.SHAPE, track_number, index_time) == Fusion_Track(FUSION_TRACK.SHAPE_ATTRIBUTE.SHAPE, init_track_number, index_time)
                                    ...
                                    % [x, y, vx, vy, heading angular rate]' + [heading angle, width, length, classification, motion]'
                                    X_est(TRACKING.REL_POS_X, init_track_number, index_time) = Fusion_Track(FUSION_TRACK.MEASURE.REL_POS_X, track_number, index_time);
                                    X_est(TRACKING.REL_POS_Y, init_track_number, index_time) = Fusion_Track(FUSION_TRACK.MEASURE.REL_POS_Y, track_number, index_time);
                                    X_est(TRACKING.REL_VEL_X, init_track_number, index_time) = Fusion_Track(FUSION_TRACK.MEASURE.REL_VEL_X, track_number, index_time);
                                    X_est(TRACKING.REL_VEL_Y, init_track_number, index_time) = Fusion_Track(FUSION_TRACK.MEASURE.REL_VEL_Y, track_number, index_time);
...
if TARGET_PRED_IMM_UKF == 1
    ...
    for track_number = 1:Traffic_Number
        for index_time = 1:length(sim_time)
            ...
            x=Fusion_Track(FUSION_TRACK.MEASURE.REL_POS_X, track_number, index_time);
            y=Fusion_Track(FUSION_TRACK.MEASURE.REL_POS_Y, track_number, index_time);
            v=-norm([Fusion_Track(FUSION_TRACK.MEASURE.REL_VEL_Y, track_number, index_time) Fusion_Track(FUSION_TRACK.MEASURE.REL_VEL_X, track_number, index_time)],2);
...
%     if IMAGE_CHANNEL == 24
%         for index_time = 1:length(sim_time)
%             v_e = norm([Fusion_Track(FUSION_TRACK.MEASURE.REL_VEL_Y, track_number, index_time) Fusion_Track(FUSION_TRACK.MEASURE.REL_VEL_X, track_number, index_time)],2);
%             ey_ini = 0;
%             WR_ESR = 0.9 - ey_ini;
%             WR_ELCR = 3.5 - ey_ini;
