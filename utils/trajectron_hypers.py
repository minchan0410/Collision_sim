...
def get_traj_hypers(config=None):
    ...
    # Always use MAT timebase from configs/train.yaml.
    mat_cfg = _load_train_yaml_required()
    if "data_dt" not in mat_cfg:
        raise KeyError("'data_dt' is missing in configs/train.yaml")
    data_dt = float(mat_cfg["data_dt"])
    if data_dt <= 0:
        raise ValueError(f"'data_dt' must be positive in configs/train.yaml, got {data_dt}")
    hypers['dt'] = data_dt
    ...
    if ph_cfg is None:
        ...
        if pred_sec is not None:
            ...
            raise KeyError("Set either 'prediction_horizon' or 'prediction_sec' in configs/train.yaml")
        ...
        if history_sec is not None:
            ...
            if (min_hist_sec is None) and (max_hist_sec is None):
                raise KeyError(
                    ...
                    "or explicit history lengths in configs/train.yaml"
                )
