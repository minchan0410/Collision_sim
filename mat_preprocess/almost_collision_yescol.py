...
def build_common_frames(df: pd.DataFrame) -> pd.DataFrame:
    ...
    merged[["x1", "y1", "x2", "y2"]] = merged[["x1", "y1", "x2", "y2"]].interpolate(
...
def _vectorized_rect_corners(
    cx: np.ndarray, cy: np.ndarray, yaw: np.ndarray, length: float, width: float, rear_axle_offset: float
) -> np.ndarray:
...
def _vectorized_obb_overlap(corners_a: np.ndarray, corners_b: np.ndarray) -> np.ndarray:
    ...
    axes = np.concatenate([axes_a, axes_b], axis=1)
...
def find_first_collision_index(
    merged: pd.DataFrame, car_length: float, car_width: float, rear_axle_offset: float
) -> Optional[int]:
...
def extract_window_frames(
    ...
    if collision_idx is None:
        raise RuntimeError("No collision detected (OBB overlap not found).")
    ...
    return selected_frames, anchor_frame, anchor_distance, anchor_type, True
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Extract collision-only windows (OBB overlap) from 2-track TXT files listed in manifest.csv."
    )
    ...
    parser.add_argument("--out-dir", default="mat_preprocess/mat_txt/collision_yescol_002", help="Output folder.")
    parser.add_argument("--before-sec", type=float, default=1.5, help="Window seconds before collision.")
    parser.add_argument("--after-sec", type=float, default=0.1, help="Window seconds after collision.")
