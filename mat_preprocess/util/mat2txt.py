...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="Sampling interval in seconds. If omitted, uses configs/train.yaml:data_dt",
    )
...
def main() -> None:
    ...
    if args.target_dt is None:
        target_dt = _load_data_dt_from_mat_yaml()
        target_dt_source = "configs/train.yaml:data_dt"
    else:
        target_dt = float(args.target_dt)
        target_dt_source = "--target-dt"
    if target_dt <= 0:
    ...
    # ?„ë¡œ?¸ìŠ¤ ?ì„± ë°©ì‹??'spawn'?¼ë¡œ ê°•ì œ?˜ì—¬ ë©”ëª¨ë¦?ê¼¬ìž„ ë°?C ëª¨ë“ˆ ì¶©ëŒ ë°©ì?
    ...
    if use_tqdm:
        ...
        print("[WARN] tqdm is not available; using text progress output.", flush=True)
    ...
    with manifest_path.open("w", encoding="utf-8", newline="") as f_manifest:
        ...
        for idx, (mat_path, source_rel) in enumerate(resolved, start=1):
            ...
            p.start()
            p.join()  # ?ì‹ ?„ë¡œ?¸ìŠ¤ê°€ ì¢…ë£Œ???Œê¹Œì§€ ?€ê¸?

            if p.exitcode != 0:
                # ?ì‹ ?„ë¡œ?¸ìŠ¤ê°€ Segfault ?±ìœ¼ë¡?ë¹„ì •??ì¢…ë£Œ??ê²½ìš°
                ...
                # ?•ìƒ?ìœ¼ë¡??„ë£Œ?˜ì—ˆ?¼ë‚˜ ?Œì´???ˆë²¨??Exception???ˆì„ ???ˆìŒ
                if not q.empty():
                    out = q.get()
            ...
            if result.status == "OK":
                ...
                if "CRASH" in result.status:
                    warn_msg = f"[WARNING] severe corrupted file skipped (Segfault): {source_rel}"
                    if pbar is not None:
                        pbar.write(warn_msg)
                    else:
                        print(warn_msg)
    ...
    print(f"  target_dt(sec): {target_dt} ({target_dt_source})")
