...
def main():
    ...
    ap.add_argument(
        ...
        help="Required field names to validate in MAT data struct.",
    )
    args = ap.parse_args()

    required_fields = [str(x).strip() for x in args.required_fields if str(x).strip()]
    if not required_fields:
        raise ValueError("--required-fields must contain at least one field name.")
        ...
        while active_tasks or tasks_iter is not None:
            ...
            while tasks_iter is not None and len(active_tasks) < MAX_QUEUE:
                try:
                    p = next(tasks_iter)
                    r = p.relative_to(in_root).as_posix()
                    fut = ex.submit(scan_one, str(p), r, args.target_var, required_fields)
                    active_tasks[fut] = r
                        ...
                        pbar.set_postfix(
                            {
                                ...
                                "no_tr": len(no_tr),
                                "unread": len(unread),
                                "folder": f"{f}:{folder_done[f]}/{folder_total[f]}",
                            },
                            refresh=False,
                        )
                else:
    ...
    with (out_dir / "summary.txt").open("w", encoding="utf-8") as file:
        ...
        file.write(f"required_fields: {','.join(required_fields)}\n")
