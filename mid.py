...
class MID():
    ...
    @staticmethod
    def _write_scene_traj_csv(csv_path, node_viz_data, current_t=None):
        """
        Save one CSV per scene visualization.

        Simplified row format:
        - agent=0 is Ego, agent=1 is Target.
        - sample=0 is input history + GT future.
        - sample=1..N is input history + generated future sample.
        - frame is the scene timestep when available.
        """
        fieldnames = ["agent", "sample", "frame", "x", "y"]
        rows = []
        ...
        for agent_index, rec in enumerate(node_viz_data):
            ...
            frame0 = 0 if current_t is None else int(current_t) - history_len + 1
    ...
    def _visualize_epoch(self, epoch):
        ...
        viz_save_images = bool(getattr(self.config, "viz_save_images", True))
        ...
        for scene_order, scene_idx in enumerate(scene_indices, start=1):
            ...
            if viz_csv_dir != "":
                ...
                self._write_scene_traj_csv(csv_path, node_viz_data, current_t=t)
            plt.close(fig)
    ...
    def _needs_eval_data(self):
        if bool(getattr(self.config, "eval_mode", False)):
            return True

        try:
            eval_every = int(getattr(self.config, "eval_every", 0) or 0)
        except Exception:
            eval_every = 0

        return eval_every > 0 or bool(getattr(self.config, "viz_enabled", False))

    def _build_dir(self):
        self.model_dir = osp.join("./experiments", self.config.exp_name)
        os.makedirs(self.model_dir, exist_ok=True)

        if bool(getattr(self.config, "tensorboard_enabled", False)):
            self.log_writer = SummaryWriter(log_dir=self.model_dir)
        else:
            self.log_writer = NoOpSummaryWriter()
        ...
        if bool(getattr(self.config, "file_log_enabled", False)):
            handler = logging.FileHandler(log_dir)
            handler.setLevel(logging.INFO)
            self.log.addHandler(handler)
        elif not any(isinstance(handler, logging.NullHandler) for handler in self.log.handlers):
